/*
 * mars.js — MARS 分组密码（cat:'modern'，encode/decode）。
 *
 * 算法照 IBM MARS 规范（Burwick/Coppersmith/D'Avignon/Gennaro/Halevi/Jutla/
 * Matyas/O'Connor/Peyravian/Safford/Zunic, 1998）与 pedroalbanese/mars (Go) 参考
 * 结构逐行一致，官方向量对拍 Crypto++ TestData/marsval.dat（128/192/256 位密钥全过）。
 *
 * 结构：分组 128 位，密钥 128-448 位（本工具支持 128/192/256），32 轮。
 *   1) 密钥编排：15 字 T 数组，4 次「线性扩展 + 4 轮搅拌」各产出 10 个密钥字，
 *      再修正奇数下标的乘法密钥字（B 表 + 掩码）。
 *   2) 加密：加 K[0..3] → 8 轮前向混合（F_MIX，间隔加 D/B）→ 16 轮加密核心
 *      （CORE，乘法取高位，轮密钥成对）→ 8 轮后向混合（B_MIX，间隔减 D/B）→ 减 K[36..39]。
 *   3) 解密为完全逆序（CORE_INV + 反向 F/B_MIX 角色）。
 * 字节序：块 4×32 位字 little-endian（Go 参考 littleEndian=true）。
 *
 * 红线：照规范/参考逐行不编造；纯本地；core 层零 UI 依赖（仅 registry）。
 *   载入自校验 Crypto++ marsval.dat 官方向量（key=8000.., pt=0 → b3e2ad5608ac1b6733a7cb4fdf8f9952）。
 *
 * 契约：register({ id:"mars", cat:"modern", name, desc, params, encode, decode })。
 *   明文/密文/密钥均 hex；ECB 多块；encode 加密 / decode 解密。
 */
import { register } from "./registry.js";

// 512 项 S 盒（前 256 = S0 段，后 256 = S1 段）——照 Crypto++ marss.cpp 逐项
...
// 奇偶修正用的 B 表
const BTAB = [0xa4a8d57b, 0x5b5d193b, 0xc8a8309b, 0x73f9a978];

function rotl32(x, n) { return ((x << n) | (x >>> (32 - n))) >>> 0; }
function rotr32(x, n) { return ((x >>> n) | (x << (32 - n))) >>> 0; }
function mult32(a, b) { return Number((BigInt(a) * BigInt(b)) & 0xffffffffn); }
function le32(b, off) { return (b[off] | (b[off + 1] << 8) | (b[off + 2] << 16) | (b[off + 3] << 24)) >>> 0; }
function wrLE32(out, off, v) {
  out[off] = v & 0xff; out[off + 1] = (v >> 8) & 0xff; out[off + 2] = (v >> 16) & 0xff; out[off + 3] = (v >> 24) & 0xff;
}

const S = (n) => SBOX[n & 0x1ff];
const S0 = (n) => SBOX[n & 0xff];
const S1 = (n) => SBOX[(n & 0xff) + 256];

// ---- 密钥编排：返回 40 个扩展密钥字（照原版 MARS 规范）----
function keySchedule(keyBytes) {
  const n = keyBytes.length / 4;
  const t = new Array(15).fill(0);
  for (let i = 0; i < n; i++) t[i] = le32(keyBytes, i * 4);
  t[n] = n; // T[n] = n（其余 0）
  ...
  for (let i = 5; i < 37; i += 2) {
    ...
    const m = maskGen(w);
    const r = K[i - 1] & 0x1f;
    K[i] = (w ^ (rotl32(BTAB[j], r) & m)) >>> 0;
  }
  return K;
}

// MASK 生成（Brian Gladman/Shai Halevi 技法）
function maskGen(w) {
  let m = (~w ^ (w >>> 1)) >>> 0;
  m &= 0x7fffffff;
  m &= (m >>> 1) & (m >>> 2);
...
// 前向混合 F_MIX（就地操作 4 个 32 位字，参数为数组下标）
function fMix(s, A, B, C, D) {
  s[B] ^= S0(s[A]);
  s[B] = (s[B] + S1(rotr32(s[A], 8))) >>> 0;
  s[C] = (s[C] + S0(rotr32(s[A], 16))) >>> 0;
  s[A] = rotr32(s[A], 24);
  s[D] ^= S1(s[A]);
}
// 后向混合 B_MIX
function bMix(s, A, B, C, D) {
  s[B] ^= S1(s[A]);
  s[C] = (s[C] - S0(rotl32(s[A], 8))) >>> 0;
  s[D] = (s[D] - S1(rotl32(s[A], 16))) >>> 0;
  s[A] = rotl32(s[A], 24);
  s[D] ^= S0(s[A]);
}
// 加密核心 CORE
function fKtr(s, A, B, C, D, k1, k2) {
  let m = (s[A] + k1) >>> 0;
  s[A] = rotl32(s[A], 13);
  let r = rotl32(mult32(s[A], k2), 5);
  s[C] = (s[C] + rotl32(m, r & 0x1f)) >>> 0;
  let l = (S(m) ^ r) >>> 0;
  r = rotl32(r, 5);
  l = (l ^ r) >>> 0;
  s[D] ^= r;
  s[B] = (s[B] + rotl32(l, r & 0x1f)) >>> 0;
}
// 解密核心 CORE_INV
function rKtr(s, A, B, C, D, k1, k2) {
  let r = mult32(s[A], k2);
  s[A] = rotr32(s[A], 13);
  let m = (s[A] + k1) >>> 0;
  r = rotl32(r, 5);
...
function encryptBlock(bytes, K) {
  const s = [le32(bytes, 0), le32(bytes, 4), le32(bytes, 8), le32(bytes, 12)];
  s[0] = (s[0] + K[0]) >>> 0; s[1] = (s[1] + K[1]) >>> 0;
  s[2] = (s[2] + K[2]) >>> 0; s[3] = (s[3] + K[3]) >>> 0;
  // 前向混合 8 轮
  fMix(s, 0, 1, 2, 3); s[0] = (s[0] + s[3]) >>> 0;
  fMix(s, 1, 2, 3, 0); s[1] = (s[1] + s[2]) >>> 0;
  fMix(s, 2, 3, 0, 1);
  fMix(s, 3, 0, 1, 2);
  fMix(s, 0, 1, 2, 3); s[0] = (s[0] + s[3]) >>> 0;
  fMix(s, 1, 2, 3, 0); s[1] = (s[1] + s[2]) >>> 0;
  fMix(s, 2, 3, 0, 1);
  fMix(s, 3, 0, 1, 2);
  // 加密核心 16 轮（前 8 轮正序，后 8 轮反序）
  fKtr(s, 0, 1, 2, 3, K[4], K[5]); fKtr(s, 1, 2, 3, 0, K[6], K[7]);
  fKtr(s, 2, 3, 0, 1, K[8], K[9]); fKtr(s, 3, 0, 1, 2, K[10], K[11]);
  fKtr(s, 0, 1, 2, 3, K[12], K[13]); fKtr(s, 1, 2, 3, 0, K[14], K[15]);
  fKtr(s, 2, 3, 0, 1, K[16], K[17]); fKtr(s, 3, 0, 1, 2, K[18], K[19]);
  fKtr(s, 0, 3, 2, 1, K[20], K[21]); fKtr(s, 1, 0, 3, 2, K[22], K[23]);
  fKtr(s, 2, 1, 0, 3, K[24], K[25]); fKtr(s, 3, 2, 1, 0, K[26], K[27]);
  fKtr(s, 0, 3, 2, 1, K[28], K[29]); fKtr(s, 1, 0, 3, 2, K[30], K[31]);
  fKtr(s, 2, 1, 0, 3, K[32], K[33]); fKtr(s, 3, 2, 1, 0, K[34], K[35]);
  // 后向混合 8 轮
  bMix(s, 0, 1, 2, 3);
  bMix(s, 1, 2, 3, 0); s[2] = (s[2] - s[1]) >>> 0;
  bMix(s, 2, 3, 0, 1); s[3] = (s[3] - s[0]) >>> 0;
  bMix(s, 3, 0, 1, 2);
  bMix(s, 0, 1, 2, 3);
  bMix(s, 1, 2, 3, 0); s[2] = (s[2] - s[1]) >>> 0;
  bMix(s, 2, 3, 0, 1); s[3] = (s[3] - s[0]) >>> 0;
  bMix(s, 3, 0, 1, 2);
  s[0] = (s[0] - K[36]) >>> 0; s[1] = (s[1] - K[37]) >>> 0;
  s[2] = (s[2] - K[38]) >>> 0; s[3] = (s[3] - K[39]) >>> 0;
  const out = new Uint8Array(16);
  wrLE32(out, 0, s[0]); wrLE32(out, 4, s[1]); wrLE32(out, 8, s[2]); wrLE32(out, 12, s[3]);
  return out;
}

function decryptBlock(bytes, K) {
  const s = [le32(bytes, 0), le32(bytes, 4), le32(bytes, 8), le32(bytes, 12)];
  s[0] = (s[0] + K[36]) >>> 0; s[1] = (s[1] + K[37]) >>> 0;
  s[2] = (s[2] + K[38]) >>> 0; s[3] = (s[3] + K[39]) >>> 0;
  // 前向混合 8 轮（角色反序）
  fMix(s, 3, 2, 1, 0); s[3] = (s[3] + s[0]) >>> 0;
  fMix(s, 2, 1, 0, 3); s[2] = (s[2] + s[1]) >>> 0;
  fMix(s, 1, 0, 3, 2);
  fMix(s, 0, 3, 2, 1);
  fMix(s, 3, 2, 1, 0); s[3] = (s[3] + s[0]) >>> 0;
  fMix(s, 2, 1, 0, 3); s[2] = (s[2] + s[1]) >>> 0;
  ...
  fMix(s, 0, 3, 2, 1);
  // 逆核心 16 轮
  rKtr(s, 3, 2, 1, 0, K[34], K[35]); rKtr(s, 2, 1, 0, 3, K[32], K[33]);
  rKtr(s, 1, 0, 3, 2, K[30], K[31]); rKtr(s, 0, 3, 2, 1, K[28], K[29]);
  rKtr(s, 3, 2, 1, 0, K[26], K[27]); rKtr(s, 2, 1, 0, 3, K[24], K[25]);
  rKtr(s, 1, 0, 3, 2, K[22], K[23]); rKtr(s, 0, 3, 2, 1, K[20], K[21]);
  rKtr(s, 3, 0, 1, 2, K[18], K[19]); rKtr(s, 2, 3, 0, 1, K[16], K[17]);
  rKtr(s, 1, 2, 3, 0, K[14], K[15]); rKtr(s, 0, 1, 2, 3, K[12], K[13]);
  rKtr(s, 3, 0, 1, 2, K[10], K[11]); rKtr(s, 2, 3, 0, 1, K[8], K[9]);
  rKtr(s, 1, 2, 3, 0, K[6], K[7]); rKtr(s, 0, 1, 2, 3, K[4], K[5]);
  // 后向混合 8 轮（角色反序）
  bMix(s, 3, 2, 1, 0);
  bMix(s, 2, 1, 0, 3); s[1] = (s[1] - s[2]) >>> 0;
  bMix(s, 1, 0, 3, 2); s[0] = (s[0] - s[3]) >>> 0;
  bMix(s, 0, 3, 2, 1);
  bMix(s, 3, 2, 1, 0);
  bMix(s, 2, 1, 0, 3); s[1] = (s[1] - s[2]) >>> 0;
  bMix(s, 1, 0, 3, 2); s[0] = (s[0] - s[3]) >>> 0;
...
function marsRun(text, p, dir) {
  const keyHex = cleanHex(p && p.key);
  const kb = keyHex.length / 2;
  if (![16, 24, 32].includes(kb)) throw new Error("MARS 密钥须为 16/24/32 字节（32/48/64 hex）");
  const K = keySchedule(hexToBytes(keyHex));
  const dataHex = cleanHex(text);
  if (dataHex.length % 32 !== 0) throw new Error("数据须为 16 字节（32 hex）的整数倍（ECB 分组）");
  let out = "";
  for (let i = 0; i < dataHex.length; i += 32) {
    const bytes = hexToBytes(dataHex.slice(i, i + 32));
    const res = dir === "enc" ? encryptBlock(bytes, K) : decryptBlock(bytes, K);
    out += bytesToHex(res);
  }
  return out;
}

// 载入自校验：Crypto++ marsval.dat 官方向量
(() => {
  const K = keySchedule(hexToBytes("80000000000000000000000000000000"));
  const c = encryptBlock(new Uint8Array(16), K);
  if (bytesToHex(c) !== "b3e2ad5608ac1b6733a7cb4fdf8f9952") {
    throw new Error("MARS 自检失败（key=8000.. pt=0 ≠ b3e2ad5608ac1b6733a7cb4fdf8f9952）");
  }
})();
...
register({
  ...
  params: [
    {
      key: "keyBits", label: "密钥长度", type: "select", default: "128",
      options: [
        { value: "128", label: "MARS-128" },
        { value: "192", label: "MARS-192" },
        { value: "256", label: "MARS-256" },
      ],
    },
    { key: "key", label: "密钥 (hex)", type: "text", default: "00000000000000000000000000000000", placeholder: "128 位: 32 hex / 192 位: 48 hex / 256 位: 64 hex" },
  ],
  encode: (text, p) => marsRun(text, p, "enc"),
  decode: (text, p) => marsRun(text, p, "dec"),
});

export { encryptBlock, decryptBlock, keySchedule, fMix, bMix, fKtr, rKtr };
